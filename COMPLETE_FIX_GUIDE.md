...
The backend has been updated to compile and run on Java 21 (LTS). Changes made:

- Updated `backend/pom.xml` property `java.version` from `11` to `21`.
- Switched the maven-compiler-plugin configuration to use `<release>21</release>` and enabled preview compiler args.
...
# Build the backend (skip tests for faster feedback)
...
- This repo's CI/build agents must have JDK 21 available. If CI uses older JDKs, update their configuration or use a tool like SDKMAN/Adoptium to install JDK 21.
- If you rely on preview features, remove `--enable-preview` after adjusting source code to non-preview APIs or when features become standard.
- **Password:** `password123`
